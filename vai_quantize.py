...
parser.add_argument(
    '--model_path',
    default="runs/best_state_dict.pt",
    help='Path to trained model checkpoint. Eg: runs/best_state_dict.pt'
)
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help='Number of classes - only needed if loading from state_dict'
)
...
parser.add_argument(
    ...
    default='relu',
    type=str,
    help='Activation function (relu, silu, etc.) - only needed if loading from state_dict'
)
...
def load_model(model_path, version='n', num_classes=1, activation='relu'):
    ...
    # Fuse Conv and BatchNorm for better quantization accuracy
...
def quantization(title='optimize',
    ...
    # Always inspect model for DPU compatibility before quantization
    print("\n" + "="*60)
    print("INSPECTING MODEL FOR DPU COMPATIBILITY")
    print("="*60)
    ...
    # Create dummy input for inspection
    dummy_input = torch.randn([1, 3, args.img_height, args.img_width], device=device)
    
    # Inspect the fused model
    print(f"Inspecting model with target: {target}")
    print(f"Input shape: [1, 3, {args.img_height}, {args.img_width}]")
    inspector.inspect(model, (dummy_input,), device=device, output_dir="inspect_result", image_format="png")
