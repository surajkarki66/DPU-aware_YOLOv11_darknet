...
def apply_dfl(dist_logits: np.ndarray, reg_max: int) -> np.ndarray:
    if reg_max <= 1:
        b, _, _, a = dist_logits.shape
        return dist_logits.reshape(b, 4, a)
    t = torch.from_numpy(dist_logits).float()
    t = t.permute(0, 1, 3, 2).contiguous()
    t = t.softmax(-1)
    w = torch.arange(reg_max, dtype=t.dtype, device=t.device)
    return (t * w).sum(-1).numpy()
...
def stretch_meta(h0: int, w0: int, imgsz: int) -> dict:
    """Map boxes from square resize (e.g. DPU ``cv2.resize`` to imgsz×imgsz) back to original image."""
    return {"h0": h0, "w0": w0, "imgsz": imgsz, "layout": "stretch"}
...
class DetectYOLO:
    ...
    def decode(self, outputs: list[np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
        ...
        pred_dist = apply_dfl(dist_logits, self.reg_max)
    ...
    def postprocess(self, xyxy: np.ndarray, scores: np.ndarray, meta: dict) -> np.ndarray:
        ...
        if meta.get("layout") == "stretch":
            imgsz = float(meta["imgsz"])
            det[:, [0, 2]] = det[:, [0, 2]] * (float(meta["w0"]) / imgsz)
            det[:, [1, 3]] = det[:, [1, 3]] * (float(meta["h0"]) / imgsz)
        else:
...
class OBB26YOLO:
    ...
    def postprocess(self, xywh: np.ndarray, scores: np.ndarray, angles: np.ndarray, meta: dict) -> np.ndarray:
        ...
        if meta.get("layout") == "stretch":
            imgsz = float(meta["imgsz"])
            sx = float(meta["w0"]) / imgsz
            sy = float(meta["h0"]) / imgsz
            det[:, 0] *= sx
            det[:, 1] *= sy
            det[:, 2] *= sx
            det[:, 3] *= sy
...
def resolve_source_from_data(data_arg: str | None) -> Path:
    ...
    if isinstance(yaml_file, (list, tuple)):
        yaml_file = yaml_file[0]
    yaml_path = Path(str(yaml_file)).resolve()
...
def apply_quant_meta(args: argparse.Namespace) -> None:
    ...
    if not isinstance(cfg, (list, tuple)) or len(cfg) < 4:
        raise ValueError(f"quant_meta pkl must be a sequence of at least 4 values, got {type(cfg)}")

    tensor_no, tensor_stride, tensor_ch, tensor_nc = cfg[0], cfg[1], cfg[2], cfg[3]
    task = cfg[4] if len(cfg) >= 5 else None

    args.reg_max = int(tensor_ch)
    args.nc = int(tensor_nc)

    if isinstance(tensor_stride, torch.Tensor):
        stride_vals = tensor_stride.detach().cpu().tolist()
    elif isinstance(tensor_stride, (list, tuple)):
        stride_vals = list(tensor_stride)
    else:
        stride_vals = [tensor_stride]

    args.strides = ",".join(str(int(s)) for s in stride_vals)
    print(
        f"Loaded quant meta from {args.quant_meta}: tensor_no={int(tensor_no)}, reg_max={args.reg_max}, "
        f"nc={args.nc}, strides={args.strides}"
        + (f", task={task!r}" if task is not None else "")
    )
    if task is not None and hasattr(args, "task") and args.task is not None and str(task).lower() != str(args.task).lower():
        print(f"[WARN] Config task={task!r} differs from CLI --task={args.task!r}")


def print_and_save_metrics(results: dict[str, float], save_dir: Path, task_title: str) -> None:
